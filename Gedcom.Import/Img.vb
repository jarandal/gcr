...
Public Class Img
    ...
    Public Shared Function proportionalResize_oldVersion(ByVal src As Bitmap, ByVal MaxWidth As Integer, ByVal MaxHeight As Integer) As Bitmap
    ...
    Public Shared Function proportionalResize(ByVal loBMP As Bitmap, ByVal width As Integer, ByVal height As Integer) As Bitmap

        Dim w As Integer = loBMP.Width
        Dim h As Integer = loBMP.Height

        Dim lnRatio As Decimal = 0
        Dim newwidth As Integer = 0
        Dim newHeight As Integer = 0

        If (loBMP.Width > loBMP.Height) Then
            lnRatio = CDec(width / loBMP.Width)
            newwidth = width
            Dim lnTemp As Decimal = loBMP.Height * lnRatio
            newheight = CInt(lnTemp)
        Else
            lnRatio = CDec(height / loBMP.Height)
            newHeight = height
            Dim lnTemp As Decimal = CDec(loBMP.Width * lnRatio)
            newwidth = CInt(lnTemp)
        End If

        ' Create new Bitmap at new dimensions
        Dim result As Bitmap = New Bitmap(newwidth, newHeight)
        Dim g As Graphics = Graphics.FromImage(CType(result, Image))
